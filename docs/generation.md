...
copies from upstream at the SHA recorded in `flipcash2.lock`.

## Sync

`sync-protos.sh` clones `flipcash2-protobuf-api`, checks out the requested ref, copies only the
contract `.proto` files (upstream's `buf.yaml`, `buf.lock`, and `buf.gen.yaml` describe how the
*contract* repo builds Go, and are not part of what this SDK ships), and rewrites
`flipcash2.lock`.

**Upstream already owns the namespace here.** The flipcash2 protos declare
`com.codeinc.flipcash.gen.*` themselves, so unlike the OCP repo there is nothing to rewrite.
`sync-protos.sh` verifies the `java_package` on every synced file instead and fails the sync if
one is missing — if upstream ever drops or changes it, the app's imports would otherwise move
silently.
...
1.5.0 was tested and differs only in line wrapping, no API change — so the bump is safe, but it
...
`Sources/Flipcash2ClientProtocol/` is committed, because SPM ships plain source: for the Swift
half, the generated code *is* the artifact. That makes it possible to change a `.proto` and
forget to regenerate, so CI re-runs `scripts/generate-swift.sh` and fails if the tree moves.
...
floating `from:` requirement — so the output moves when *grpc-swift-2* releases, with the
plugin's own version unchanged. 2.2.1 added `Sendable` to the metadata enums; 2.3.0 added `type:`
to every `MethodDescriptor`. That is hundreds of changed lines for no contract change.
...
The Swift files here are named straight from their protos — `messaging_v1_messaging_service.*`,
not the `flipcash_`-prefixed names the iOS app used to produce. That prefix was an artifact of
module merging: the app generated both protocols into one `FlipcashAPI` module, where the OCP
messaging service collided, so its `Scripts/run` renamed the flipcash2 side. With a package per
protocol the collision cannot happen. The Swift type names (`Flipcash_Messaging_V1_*` against
`Ocp_Messaging_V1_*`) never differed.
...
- **Kotlin**: `./gradlew build` (protoc plus compiling the result — if the generated code does
  ...
  summary.
- **Swift**: `swift build`, then regenerate and `git diff --quiet -- Sources`. The toolchain
  cache key is `scripts/toolchain.env`, so a bump rebuilds the plugins and nothing else does.
